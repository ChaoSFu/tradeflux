...
更新阻塞，也不该反过来阻塞全量更新。**纯手动触发**（2026-08-24 移除了此前的 `mon-fri
09:26` 定时任务，见下方"09:26 定时任务移除记录"），页面按钮点击 → `POST /refresh` →
后台线程跑 `run_refresh()`，同一时刻只允许一个实例（同一把锁）。失败不重试——非关键
路径，用户随时可在页面手动点刷新补一次。

**已知运营缺口 + 2026-08-23 产品定位校准**：全天状态推进完全依赖用户手动点"刷新数据
并重新评估"，没有任何自动触发。这意味着如果没人在交易时段主动刷新，H1/L1 回踩结构的
