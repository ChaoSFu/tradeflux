...
- **交易日历会漏天**：2026-09-07 拉的那份没有 09-04（那天明明开市）。缓存只在跨天时刷新，
  漏掉的那天会让所有按日历判「相邻」的地方在两边断开——龙版传媒 08-31~09-07 的 6 连板被数成 4 板。
  现在 `trading_calendar.get_trading_days` 每次都用上证日线把漏掉的交易日补上（只补不删）
- **日K dump / 10 年存档里停牌期间没有行**：这正是认停牌的依据（`suspension_service.suspended_days_from_rows`）

**停牌股的实时行情**（2026-09-15 核实）：腾讯对停牌股照样给一条行情，现价 = 昨收、成交量 0、
日期是今天。所以「价格 <= 0 算停牌」拦不住它；2026-09-15 之前收盘后的行情兜底会据此造出一根
「涨跌 0、成交量 0」的假 bar（龙版传媒 09-09~09-11）。现在 `kline_bar_from_quote` 见零成交
就不补，日更收盘那一跑把它记进停牌日表
...
15. **派生状态不落进事实表。** 2026-09-04 上 Price Lifecycle v1 状态机时定死的：
    `LeaderCycleSnapshot` 继续 DAILY FACT ONLY，状态一律从历史事实 replay。

    理由跟 `RegulatoryStatusDaily`「派生事件不落库」是同一条：**阈值以后一定会改**。
    把 `CROSS_SUCCESS` 冻进历史事实表，price_v1 换成 v2 之后整段历史就变成旧口径
    数据，再也回答不了「新口径下当时该是什么状态」。replay 的代价是几十只 × 60 天，
    可以忽略；真有性能问题再加 materialized cache，不要反过来先污染事实层。

    配套的两条：口径带 `FORMULA_VERSION`，版本对不上直接报错而不是静默按旧的算；
    拍出来的常量（「连续几次」这类）做成模块级常量，换阈值时历史能整段重算。

16. **「连续 N 个交易日」不能靠「过滤后相邻」。** 同一天踩到的：状态机把不可用的
    行过滤掉之后，第 3 天和第 5 天在列表里直接相邻，于是隔着一个交易日的两次破位
    被当成「连续两日」。

    中间那天可能是**停牌**（那两侧确实是连续的交易 observation），也可能是**数据
    缺口**（那天交易了但我们没数据，不连续）。这两件事在快照层分不出来——
    `suspended_map` 目前压根没传给 `build_snapshots`。

    分不出来就保守：传入观测日历，证明不了相邻就不触发。代价是跨停牌的信号会漏一
    次，但要留意**漏报的方向**——对「退出买入池」这类规则，漏报是危险方向，所以
    price_v1 里另配了一条不依赖连续性的单日规则（收盘跌破 MA10 即走弱）兜底。

    这跟坑 14（窗口切开连板段）是同一个家族：**时间轴上的相邻关系，必须用日历
    证明，不能用数组下标推断。**

    **同一天这个错犯了三次**，一次比一次靠近底层：状态机的「连续两个
    observation」→ 快照空洞脚本的复权校验 → 最后发现它一直躺在最底层的
    `board_streaks()` 里。第三次最贵：它让 603065 被数成 6 连板（补齐空洞后
    真实是 2 板），而我据此把 `board_count_60d` 从正确的 2 改成了错误的 6，
    还推翻了一次原本正确的人工核实。

    **修完第一次之后没去搜同一模式的其它出现，是这三次里唯一可以避免的部分。**
    发现一个模式错误时，`grep` 一遍同族写法（`zip(xs, xs[1:])`、`xs[i-1]`、
    过滤后再取相邻）比修好眼前这一处更重要。

17. **`push2his.eastmoney.com` 时好时坏，原因未定 —— 而我为此下过四次错结论。**

    2026-09-06 的排查过程比结论更值得留。我先后认定过：

    | # | 结论 | 被什么推翻 |
    |---|---|---|
    | 1 | 请求太快触发限流 | 我们是单线程 2 秒间隔，**第一发就失败** |
    | 2 | keep-alive 复用了死连接 | 全新连接（不同 host，池里没得复用）照样被掐 |
    | 3 | 板块 `90.` 被单独挡 | 个股 `secid=1.600000` 同样被掐 |
    | 4 | **服务器出口 IP 被封** | 二十分钟后**开发机也 12/12 失败**，落在同一个节点上 |

    第 4 条我已经提交进仓库，还据此写了导出/导入的绕道方案。

    ## 已经确定的

    · 换参数、换 UA、加全套浏览器头、加 cookie、`--resolve` 钉节点 —— 全部无效
    · 同一时刻 push2delay / push2ex / datacenter / quote 网页 **200 正常**
    · 边缘 IP **是轮转的**，两台机器每次拿到的节点都可能不同
    · 节点 `101.226.30.221` 对两台机器都失败；开发机早前命中
      `14.103.188.89` / `120.79.123.125` 时成功（完整 300 根）

    ## 还不知道的

    是东财节点池大面积故障（碰运气），还是按来源的软拒绝而开发机早前恰好绕过。
    **两者处置完全不同**：前者靠重试+换节点，后者只能绕道。

    ## 方法上的教训（这条才是重点）

    判断"是不是被封"至少要三组对照 —— **换参数、换请求特征、换同域名的其它端点**，
    再加一条最常被忽略的：**换时间**。前四次错判里三次是对照不全就收敛，第四次是
    只在一个时间点取样。
