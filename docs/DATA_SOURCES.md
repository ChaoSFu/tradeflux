...
17. **`push2his.eastmoney.com` 在生产服务器上完全不可用（2026-09-06 完整验证）。**

    这是"别一见失败就认定限流"的反面教材——真验下来它不是限流，但过程中我先后
    错认过三次原因。完整的排除梯子（每一级都实测）：

    | 试的是什么 | 服务器结果 |
    |---|---|
    | 只有 `secid` 的最短 URL | `(52) Empty reply` |
    | 全套参数 | `(52)` |
    | 个股 `secid=1.600000`（不是板块专属？） | `(52)` |
    | 换新 UA | `(52)` |
    | 全套浏览器头（Sec-Fetch-*、sec-ch-ua*） | `(52)` |
    | 再加上能通的机器的整串 cookie | `(52)` |
    | `--resolve` 钉到能通的机器用的那个边缘节点 | `(52)` |
    | IPv6 | `(7)` 服务器无 IPv6 出口 |
    | **同一时刻** push2delay / push2ex / datacenter / quote 网页 | **200 ✓** |

    同一条请求在开发机上返回完整 300 根。所以跟参数、请求头、cookie、边缘节点、
    网络本身**全都无关**，是这台机器的出口被 `push2his` 单独拒绝。

    **降速、退避、换 UA、加 `Connection: close` 全部没有意义。**

    三个后果：

    · 板块指数历史只能在别的机器上导出再导入
      （`export_sector_klines.py` / `import_sector_klines.py`）。
      好在它只需要成功**一次**——往后每天那一根由 sync_boards 从 clist 顺手写，
      走的是没被拦的 push2delay。
    · `_fetch_kline_eastmoney` 和 `fetch_index_kline` 的东财兜底，在这台机器上是
      **永远走不通的死路**。有腾讯/新浪做主力所以不影响功能，但别去 debug 它。
    · 还不知道是永久封还是临时封（会不会自己解除），也不知道是东财侧拒绝还是
      阿里云→东财这段路径的问题。这两个从服务器上查不出来，但都指向同一个处置。

    **方法上的教训**：判断"是不是被封"至少要三组对照——换参数、换请求特征、换
    同域名的其它端点。少任何一组，结论都可能是错的。这次前三次判断（限流 / 
    keep-alive 死连接 / 板块被单独挡）全是在对照不全的情况下下的。
