> **注意**：本文档描述的是 **V1 legacy architecture**。
> 当前产品设计（决策链 V2）见 [docs/V2_DESIGN.md](docs/V2_DESIGN.md)。
> 两者共存：V1 页面全部保留可用，V2 在 `/v2/*` 独立路由下。
...
#### ~~强势股池（SectorPool）~~ ★ 已合并入「情绪板块」

- 原"板块强势分布"页已删除，功能 = 情绪板块的「强势股」过滤
...
#### ~~板块综合分析（SectorAnalysis）~~ ★ 已删除

- 功能与情绪板块「全部」过滤重复，已删除
- 「板块生命周期分布」条（7阶段）曾移至 趋势板块（SectorTrend）顶部；2026-09-13 起板块趋势改成
  「主升板块雷达」（`docs/SECTOR_MAINLINE.md`），分布条和板块排行榜一起折叠进页面底部的「传统多周期排名」

#### 板块趋势 · 主升板块雷达（SectorTrend）★ 2026-09-13 重做

- 在「大盘 → 板块主线 → 个股」这条链上管中间那一环：回答现在哪几个板块在主升
- 事实 → 四道闸（趋势 / 相对强度 / 生态 / 风险）→ 状态（加速 / 主升 / 点火 / 高潮 / 分歧 / 转弱 / 无 / 未知）→ 证据，不打分
- 生态走成分股日快照（跟买入检查同一个口径），不用 SectorDailySnapshot，也不用任何黑箱分
- **主升状态 ≠ 买入信号**：只描述板块状态，不给买卖建议
- 第一版是规则型状态机，阈值是工程初始值；规则、阈值出处、跟 ChatGPT 方案的取舍见 `docs/SECTOR_MAINLINE.md`
...
#### 情绪板块（SectorEmotion）★ 统一的板块分组视图

- 分组卡片布局（共享组件 `SectorGroupedView`），成员表支持**按列排序**（默认排序不变）
- 成员个股**过滤**：全部（强势池+涨停+跌停）/ 强势股（仅强势池）/ 涨跌停股（涨停+跌停）
- 已**取代并删除**原「板块强势分布 SectorPool」（=强势股过滤）与「涨跌停板块分布
  LimitMovesSectors」（=涨跌停过滤）两个页面，功能合一
...
#### ~~板块涨跌停分布（LimitMovesSectors）~~ ★ 已合并入「情绪板块」

- 原"涨跌停板块分布"页已删除，功能 = 情绪板块的「涨跌停股」过滤
