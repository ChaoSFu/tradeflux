...
# 强势池选股 prompt。2026-09-03 收窄，语义从「近期有哪些强势股」改成
# **「过去60个交易日内真正打开过连续涨停高度的高辨识度龙头」**。
#
# 只留一条：PeakBoard60D >= 4。删掉的三条 OR 召回条件——
#     近60日涨停天数>9 / 近10日涨停天数>4 / 近20日涨幅前10
# ——它们召回的是"涨得多"和"涨停频繁"，跟"打开过市场高度"是两回事。今天首板、
# 二板但历史从未 4 连板的股票不再进池，它们属于涨跌停股池。
#
# 也不再要求"主板"：创业板/科创板真达到高连板同样是高辨识度龙头。板块制度差异
# （10%/20% 涨跌停）由下游按 board_type 区分，不在召回这一层砍掉。
#
# 迁移见 pool_config_service.migrate_strong_pool_keyword()——用户自定义过的
# prompt 不会被无差别覆盖。
STRONG_POOL_KEYWORD = (
    "非ST;非退市股；非新股非次新；"
    "近60个交易日最高连板数大于3;"
...
# 收窄之前用过的默认值（含用户手动去掉"主板"的那个变体）。迁移时只覆盖这些，
# 命中不了的一律认定为真·自定义，不动。见 pool_config_service。
LEGACY_STRONG_POOL_KEYWORDS: tuple[str, ...] = (
    "主板非ST;非退市股；非新股非次新；"
    "近60个交易日最高连板数大于3或者"
    "近60个交易日涨停天数大于9或者"
    "近10个交易日涨停天数大于4或"
