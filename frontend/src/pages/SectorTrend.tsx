...
const STATE_META: Record<MainlineState, { label: string; desc: string; cls: string; dot: string }> = {
  ...
  IGNITION: { label: '点火', desc: '三道闸今天第一次全过，还差一天确认（近 3 天要有 2 天全过才算主升）',
    cls: 'text-teal-300 bg-teal-500/10 border-teal-500/30', dot: 'bg-teal-400' },
  CLIMAX: { label: '高潮', desc: '极端乖离，且出现见顶迹象（封板率低、放量滞涨、大阴、跌停），或涨停、成交也同时极端',
  ...
  WEAKENING: { label: '转弱', desc: '最近 5 天主升过，现在跌破 MA20 / 跑输上证，或跌破 MA10、5 日跑输、涨停收缩同时出现',
    cls: 'text-slate-300 bg-slate-500/15 border-slate-500/30', dot: 'bg-slate-400' },
  NONE: { label: '无', desc: '不在主线里（只过了两道闸、单日涨停爆发的也在这里，理由里写着差哪一道）',
