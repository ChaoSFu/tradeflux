...
const LIMITATIONS = [
  ...
  { title: '回踩结构识别基于离散采样，不是连续监控', body: 'H1/L1 判断依赖每次刷新拿到的价格快照，采样越稀疏越可能错过真正的高点/低点。全天状态推进完全依赖手动点刷新，没有任何自动触发（2026-08-24起，此前唯一的09:26自动刷新也已移除）——这是刻意的产品定位，不追求发现速度，判断正确性优先，不会为了提升采样密度新增高频自动轮询。' },
...
const ROADMAP_DONE = [
  ...
  '2026-08-23~24 外部评审round4驱动的10项修复：VWAP单位/涨跌停幅度语义/Mainline与Leader排名池子扩到全市场/Checklist LEADER与RISK对齐真实闸门语义/前端表格与类型修复；2026-08-24移除09:26定时任务，改为纯手动触发',
]

const ROADMAP_SKIPPED = [
  { title: '监管风险 0-100 连续分', body: '复查原始规格发现明确禁止对监管风险给出精确数值型说法，只允许四档粗分类，做连续分本身就是编造假精度，判断后不做。' },
  { title: '高频（30/60秒）自动候选快照轮询', body: '曾经评估过按结构态分级的自适应轮询方案，结合实际交易方式（只关注少量核心主线板块和龙头、买入条件严格）后撤回——判断正确性优先于发现速度，新增高频轮询会实质性提高对本就不稳定的东财接口的常态请求压力，判断后不做。此前唯一保留的09:26盘前自动刷新也在2026-08-24移除（重新审查发现"竞价窗口只有5分钟抓不住"这个理由本身不成立，见实现原理文档），现在完全是纯手动触发。' },
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-x-6 mb-4">
          <div>
            ...
            <p className="text-xs text-text-secondary leading-relaxed">
              每日更新流程（收盘15:30/盘前09:26）里新增一步：跑两路候选 Prompt（复用东方财富智能选股接口），
          ...
          <div>
            ...
            <p className="text-xs text-text-secondary leading-relaxed">
              独立 API，纯手动按钮触发（无任何自动定时），目标5-10秒完成：查活跃候选 → 按题材分组跑板块/龙头闸门 →
