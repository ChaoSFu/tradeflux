...
const ROADMAP_PLANNED = [
  { title: '候选专属日内快照引擎', body: '外部评审建议提前到最优先——复用刷新时已经拉到的报价数据写一张快照表，不需要新的外部请求，成本很低；唯一还没定的是自动定时抓取的频率，这会改变对东财接口的稳定态请求压力，需要单独确认后再加。' },
  { title: 'Golden Case 测试场景', body: '不需要等完整回测框架——先用合成的多步价格序列+真实历史案例，针对状态机核心判断函数写语义级测试，独立于下面的回测框架。' },
  ...
  { title: '东财 Prompt 解析降级监控', body: '候选召回数量异常骤降时目前没有自动告警，只能靠人工观察。' },
]
...
export default function WeakToStrongRadarGuide() {
  return (
    <div className="space-y-4 animate-fade-in">
      ...
      {/* 1. 五道闸门 */}
      <div className="card p-5">
        <SectionHead num="01" title="五道闸门" sub="候选进入雷达后依次经过五道关卡，但它们不是同一种性质——大盘/板块/龙头(非核心)命中即硬性拦截（展示BLOCK）；龙头(未决)/空间不足是软上限（压低能展示到的最高状态，不直接判死）；结构确认本身是价格驱动的进度条，不是一道会拦截谁的关卡，只是没走到那一步就自然还没到 BUYABLE。" />
